...
export default {
    ...
    created() {
        ...
        if (index > -1) {
            ...
            this.$Echo
                .join(`room.${this.currentRoom.id}`) // listen to the shared room
                .here(users => {
                ...
                .joining(user => {
                    ...
                    if (
                        ...
                        this.privateChat.selectedReceiver.id === user.id
                    ) {
                ...
                .listen("MessagePosted", e => {
                    ...
                    this.scrollToBottom(
                        document.getElementById("shared_room"),
                        true
                    );
                })
            ...
            this.$Echo
                .private(`room.${this.$root.user.id}`) // listen to user's own room (in order to receive all private messages from other users)
                .listen("MessagePosted", e => {
                    if (
                        this.privateChat.selectedReceiver &&
                        e.message.sender.id ===
    ...
    methods: {
        async getMessages(room, page = 1, loadMore = false) {
            ...
            try {
                ...
                if (loadMore) {
                    this.$nextTick(() => {
                        const el = $(
                            isPrivate ? "#private_room" : "#shared_room"
                        );
        ...
        async selectReceiver(receiver) {
            ...
            const roomId =
                this.$root.user.id > receiver.id
                    ? `${receiver.id}__${this.$root.user.id}`
                    : `${this.$root.user.id}__${receiver.id}`;
            this.privateChat.selectedReceiver = receiver;
            this.privateChat.isPrivateChatExpand = true;
            this.privateChat.roomId = roomId;
            this.$Echo
                .private(`room.${roomId}`) // this room to receive whisper events
        ...
        async selectEmoji(emoji) {
            try {
                ...
                const index = this.selectedMessage.reactions.findIndex(
                    item => item.user_id === this.$root.user.id
                );
                if (index > -1) {
        ...
        onOtherUserReaction(reaction, room) {
            ...
            listMessage =
                room === "share"
                    ? this.publicChat.message.list
                    : this.privateChat.message.list;

            const messageIndex = listMessage.findIndex(
